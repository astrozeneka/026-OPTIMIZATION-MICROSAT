...
file_path = "data/2_elephant_dominic_v2/Kuiburi_EMA_clean.str"
if __name__ == '__main__':
    GLOBAL_FILE_PATH=file_path

    loci_list = get_loci_list(file_path)
    indiv_list = get_indiv_list(file_path)
    phe_files = glob("data/aco-phe/phe-*-2.npy")
    ...
    for file in phe_files:
        index = int(file.replace("data/aco-phe\\phe-", "").replace("-2.npy", ""))
    ...
    print()
    error_df.to_csv("data/2_elephant_dominic_v2/report.csv", index=True)
